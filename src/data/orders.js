...
export const orders = [
  ...
  // ── 3번째: 애매(레터링 '필요없음' + 초콜릿) ──────
  {
    ...
    promptNote: `먼저 판단하라: 주인의 질문이 '쪽지(레터링)'에 대한 것인가?
- 아니라면(데코·베이스·토핑·생크림 등) 이 절을 완전히 무시하고 평소 규칙대로 답한다.
  예) "데코 없어도 돼?" → "데코: 필요없음." (데코는 진짜 없음이 맞다. 쪽지 이야기 금지)
- 쪽지 질문일 때만: 쪽지 정답은 "필요없음"이라는 세 글자를 케이크에 그대로 쓰는 것이다.
  "안 써도 되지?" "빼도 되지?" 에 절대 동의하지 마라 — "아니. 쪽지: '필요없음'. 그대로 기재."
  방향으로 답한다(따옴표로 문자열임을 표시). 쪽지 얘기에서 '필요없어'라는 말을 쓰지 마라.`,
    hints: [
