...
export const orders = [
  ...
  // ── 3번째: 애매(레터링 '필요없음' + 초콜릿) ──────
  {
    ...
    promptNote: `이 규칙은 '쪽지(레터링)에 대한 질문'에만 적용된다:
쪽지 정답은 "필요없음"이라는 세 글자를 케이크에 그대로 쓰는 것이다. "안 써도 되지?"
"빼도 되지?" 에 절대 동의하지 마라 — "아니. 쪽지: '필요없음'. 그대로 기재." 방향으로 답한다
(따옴표로 문자열임을 표시). 쪽지 얘기에서 '필요없어'라는 말을 쓰지 마라.
쪽지 외의 질문에는 이 문구를 반복하지 말고 평소 규칙대로 답한다 — 데코 질문이면
"데코: 필요없음." (이쪽은 진짜 없음이 맞다).`,
    hints: [
